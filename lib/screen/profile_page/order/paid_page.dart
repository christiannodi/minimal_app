...
import '../../../bloc/getorderpaid_bloc/getorder_bloc.dart';
import '../../../theme.dart';
...
class PaidPage extends StatefulWidget {
  ...
  @override
  State<PaidPage> createState() => _MustpayPageState();
}

class _MustpayPageState extends State<PaidPage> {
  @override
  void initState() {
    super.initState();

    // ✅ Hanya fetch jika state belum ada
    context.read<GetorderpaidBloc>().add(Getorderpaid());
  }
    ...
    return BlocListener<PaynowBloc, PaynowState>(
      ...
      child: Scaffold(
        ...
        body: Padding(
          ...
          child: BlocBuilder<GetorderpaidBloc, GetorderpaidState>(
            builder: (context, state) {
              if (state is GetorderpaidSuccess) {
                final listOrders =
            ...
            Align(
              ...
              child: GestureDetector(
                onTap: () async {
                  final arrived = await showDialog<bool>(
                    ...
                    builder: (BuildContext context) => Dialog(
                      shape: RoundedRectangleBorder(
                        borderRadius:
                            BorderRadius.circular(12.0), // Sudut lebih rounded
                      ),
                      elevation: 0,
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 16),
                            Text(
                              'Are you sure the item has been received?',
                              style: TextStyle(
                                fontSize: 14,
                  ...
                  if (arrived == true && context.mounted) {
                    context.read<PaynowBloc>().add(
                        Paynow(orderListDataModel.id, {"status": "Shipped"}));
                  }
                },
                // onTap: () {
                //   context.read<PaynowBloc>().add(
